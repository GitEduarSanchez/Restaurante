...
function TablaListarKardexProductos()
   {
    ...
    $this->Row(array($a++,
        ...
        utf8_decode($kardex[$i]["devolucion"]),
        utf8_decode($kardex[$i]['stockactual']),
        utf8_decode(number_format($kardex[$i]["ivaproducto"], 2, '.', ',')),
...
function TablaListarMenu()
   {
    ...
    $this->MultiCell(100,3.5,$this->SetFont('Courier','B',10).utf8_decode($detalles),0,'R');
    $this->Ln();
...
function TablaListarCombosxMoneda()
   {
    ...
    $this->SetFont('Courier','',10);  
    $this->SetTextColor(3,3,3);  // Establece el color del texto (en este caso es negro)
    $this->Row(array($a++,utf8_decode($reg[$i]["codcombo"]),portales(utf8_decode($reg[$i]["nomcombo"])),utf8_decode($simbolo.number_format($reg[$i]['precioventa'], 2, '.', ',')),$tipo = ($cambio[0]['moneda'] == "EURO" ? chr(128) : $cambio[0]['simbolo']).utf8_decode(number_format($reg[$i]['precioventa']/$cambio[0]['montocambio'], 2, '.', ',')),utf8_decode($reg[$i]['existencia']),utf8_decode($reg[$i]['ivacombo'] == 'SI' ? number_format($imp[0]["valorimpuesto"], 2, '.', ',')."%" : "(E)"),utf8_decode(number_format($reg[$i]['desccombo'], 0, '.', ',')),utf8_decode($detalles)));
       }
   }
...
################################# BLOQUE N° 5 ####################################### 
    ...
    $this->RowFacture2(array($a++,
        ...
        utf8_decode(number_format($detalle[$i]["ivaproducto"], 2, '.', ',')),
        utf8_decode($simbolo.number_format($detalle[$i]['valorneto'], 2, '.', ','))));
  }
################################# BLOQUE N° 5 #######################################  
...
function TicketCierre()
    ...
    $this->SetX(4);
    $this->CellFitSpace(66,3,utf8_decode($provincia = ($con[0]['provincia'] == '' ? "" : $con[0]['provincia'])." ".$departamento = ($con[0]['departamento'] == '' ? "" : $reg[0]['departamento'])),0,1,'C');
...
function TicketComanda()
    ...
    $this->SetFont('Courier','B',12);
    $this->SetFillColor(2,157,116);
    $this->Cell(66, 5, "TICKET DE COMANDA", 0, 0, 'C');
    $this->Ln(5);
    ...
    $this->SetX(4);
    $this->SetFont('Courier','B',8);
    $this->CellFitSpace(20,3,"Nº DE PEDIDO:",0,0,'L');
    $this->SetFont('Courier','',8);
    $this->CellFitSpace(46,3,utf8_decode($reg[0]['codpedido']."-".$reg[0]['pedido']),0,1,'L');
    ...
    $this->SetFont('Courier','',8);  
    $this->SetTextColor(3,3,3);  // Establece el color del texto (en este caso es negro)
    $this->Cell(10,3,utf8_decode($reg[$i]['cantventa']),0,0,'L');
...
function TicketVenta()
    ...
    $this->SetX(4);
    $this->SetFont('Courier','B',8);
    $this->CellFitSpace(16,3,"Nº TICKET:",0,0,'L');
    $this->SetFont('Courier','',8);
...
{  
    ...
    $this->SetX(4);
    $this->CellFitSpace(66,3,"Nº BOLETA: ".utf8_decode($reg[0]['codfactura']),0,1,'C');
...
function TablaListarVentasxTipos()
   {
    ...
    $this->SetFont('Courier','',10);  
    $this->SetTextColor(3,3,3);  // Establece el color del texto (en este caso es negro)
    $this->Row(array($a++,utf8_decode($documento = ($reg[$i]['documcliente'] == '0' ? "DOCUMENTO" : $reg[$i]['documento']).": ".$reg[$i]["dnicliente"]),utf8_decode($reg[$i]['nomcliente']),utf8_decode($reg[$i]['tlfcliente'] == '' ? "*********" : $reg[$i]['tlfcliente']),utf8_decode($reg[$i]['cantidad']),utf8_decode($simbolo.number_format($reg[$i]['totalcompras'], 2, '.', ','))));
        }
      }
